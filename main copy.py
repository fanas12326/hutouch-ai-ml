...
async def identify_task(item: Item):
    ...
    # Fetch tasks dynamically from another API
    # async with httpx.AsyncClient() as client:
    #     response = await client.post("http://127.0.0.1:8000/calculate-task", json={"id": 5}, headers={"Content-Type": "application/json"})
    #     print(response)
    #     if response.status_code != 200:
    #         raise HTTPException(status_code=response.status_code, detail="Failed to fetch tasks from the API")
    #     tasks = response.json()
    
    url = f"http://127.0.0.1:8000/calculate-task"
    # Headers as specified
    headers = {
        'Content-Type': 'application/json',
    }
    data_to_be_sent ={"id":item.id,"prompt":""}
    # Make the POST request
    response = requests.post(url, headers=headers,json=data_to_be_sent)
    ...
    def extract_relevant_task(user_prompt, tasks):
        prompt_keywords = extract_keywords(user_prompt)
        max_overlap = 0
        best_match = ("No specific task matched", [])
        for task, steps in tasks.items():
    ...
    df = load_data(filename)
    task_name, steps = extract_relevant_task(user_prompt, tasks)
    top_apps = find_top_apps(df, task_name, steps)
    print("Identified Task:", task_name)
    print("Steps:", ', '.join(steps))
    print("\nTop Matching Applications:")
